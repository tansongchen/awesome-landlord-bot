...
TEST_CASE("Strategy 2: returning same category", "[0]") {
  ...
  SECTION("Case 2.1") {
    ...
    REQUIRE(defend(&counter, last_hand) == Hand(10, 1, 4));
  }
  SECTION("Case 2.2") {
    Counter counter({0, 4, 8, 12, 16, 20, 24, 28});  // 3 5 6 7 8 9 10
    Hand last_hand(Counter({1, 5, 9, 13, 17}));      // 3 4 5 6 7
    REQUIRE(defend(&counter, last_hand) == Hand(Counter({8, 12, 16, 20, 24})));
  }
  SECTION("Case 2.3") {
    ...
    REQUIRE(defend(&counter, last_hand) ==
            Hand(Counter({3, 41, 42, 43})));  // K K K 3
  }
}
...
TEST_CASE("Strategy 4: PASS", "[0]") {
  ...
  SECTION("Case 4.1") {
    ...
    Hand last_hand(Counter({0, 1, 2, 22}));     // 3 3 3 8
    REQUIRE(defend(&counter, last_hand) ==
            Hand(7, 1, 4));  // 10 10 10 10
                             // I set the rule not to split the bomb
  }
  SECTION("Case 4.2") {
